...
import { holidayApi } from "@/api/user/holidayApi";
    ...
    const fetchHolidays = async () => {
      try {
        ...
        const responses = await Promise.all(
          categoriesOrder.map(cat => holidayApi.getAllHolidays({ category: cat }))
        );

        const formattedGroups = categoriesOrder.map((title, index) => {
          return {
            title,
            items: responses[index].data || []
          };
